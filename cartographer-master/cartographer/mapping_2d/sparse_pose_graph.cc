...
void SparsePoseGraph::AddScan(
  ...
  const transform::Rigid3d optimized_pose(
      GetLocalToGlobalTransform(trajectory_id) * transform::Embed3D(pose));  //根据优化后的信息创建轨迹
  ...
  AddWorkItem([=]() REQUIRES(mutex_) {
    ComputeConstraintsForScan(trajectory_id, insertion_submaps,             // 调用 ComputeConstraintsForScan
...
void SparsePoseGraph::ComputeConstraint(const mapping::NodeId& node_id,
  ...
  if (node_id.trajectory_id != submap_id.trajectory_id &&
    ...
    constraint_builder_.MaybeAddGlobalConstraint(                                               //调用 MaybeAddGlobalConstraint；
    ...
    if (node_id.trajectory_id == submap_id.trajectory_id ||
      ...
      const transform::Rigid2d initial_relative_pose =
          ...
          optimization_problem_.node_data()
              ...
              .point_cloud_pose;
      constraint_builder_.MaybeAddConstraint(                                                  // 调用 MaybeAddConstraint ；
          submap_id, submap_data_.at(submap_id).submap.get(), node_id,
          &trajectory_nodes_.at(node_id).constant_data->range_data.returns,
...
void SparsePoseGraph::ComputeConstraintsForScan(
  ...
  for (int trajectory_id = 0; trajectory_id < submap_data_.num_trajectories();
    ...
    for (int submap_index = 0;
      ...
      if (submap_data_.at(submap_id).state == SubmapState::kFinished) {
        ...
        ComputeConstraint(node_id, submap_id);                                            //调用 ComputeConstraint;
      }
    }
  }
  ...
  if (newly_finished_submap) {
    ...
    // old scans.
    ComputeConstraintsForOldScans(finished_submap_id);          //调用 ComputeConstraintForoldScans;
  }
