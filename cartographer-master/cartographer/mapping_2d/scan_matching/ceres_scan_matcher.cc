...
/*
CeresScanMatcher::Match（）主要就是构造约束条件，然后调用ceres库完成优化;
约束条件是如何构造的? 这个函数里有3个约束条件的构造。

所谓优化问题，一般都是将某一问题抽象（就是构造约束条件，也就是构造残差表达式），
然后转换为优化线性或非线性最小二乘问题。
*/
...
/*
激光点云占据栅格的代价函数，
当点云匹配到原地图中的自由状态的栅格时，代价增加；
当匹配到占据状态的栅格时，代价减小。
同时还有一个点云占据系数，可在配置文件中设置。
*/  
 ...
 /*
 位置代价函数，
 当估计位置与初始给定位置距离增大时，代价也增大，
 同时还有一个平移权重系数，在配置文件中可设定。
 */ 
  problem.AddResidualBlock(
...
/*
旋转代价函数，
但估计角度与初始给定位姿的角度相差增大时，代价增大，
同时还有一个旋转权重系数，在配置文件中可设定。
 */ 
  problem.AddResidualBlock(
