...
/*
GenerateExhaustiveSearchCandidates;详尽搜索所有候选;

	遍历所有scan_index;
		num_linear_x_candidate;
		num_linear_y_candidate;
	得到num_candidates;
*/
...
RealTimeCorrelativeScanMatcher::GenerateExhaustiveSearchCandidates(
  ...
  candidates.reserve(num_candidates); 	//预留容器容量；
  for (int scan_index = 0; scan_index != search_parameters.num_scans;
       ++scan_index) {
    for (int x_index_offset = search_parameters.linear_bounds[scan_index].min_x;
      ...
      for (int y_index_offset =
        ...
        candidates.emplace_back(scan_index, x_index_offset, y_index_offset,
                                search_parameters);  							 //emplace_back在容器后塞入；
      }
    }
  }
...
double RealTimeCorrelativeScanMatcher::Match(
  ...
  std::vector<Candidate> candidates =
      GenerateExhaustiveSearchCandidates(search_parameters);     			  // 1. 遍历搜寻；
  ScoreCandidates(probability_grid, discrete_scans, search_parameters,	         // 2. 统计计分；
                  &candidates);

  const Candidate& best_candidate =
      *std::max_element(candidates.begin(), candidates.end());     // 3. 找到分数最高的；
  *pose_estimate = transform::Rigid2d(
      {initial_pose_estimate.translation().x() + best_candidate.x,   // 4.根据得分最高的candidate(得分最高=概率最大)得到最优pose_estimate;
