...
/*
全局优化用；在全局地图中开个窗口，然后在窗口内进行扫描匹配;
调用 MatchWithSearchParameters
*/
...
/*
 	1.首先生成以一定的角步长生成旋转候选粒子， GenerateRotatedScans ，
      即给定一个激光扫描，以及角步长，角度从0到360°，生成旋转候选粒子； 
*/
  const std::vector<sensor::PointCloud> rotated_scans =
      GenerateRotatedScans(rotated_point_cloud, search_parameters);  

/*
	2. 将旋转候选粒子离散化， DiscretizeScans，
	   即原始的激光扫描是由浮点的（角度，距离值）组成，离散化后变成整型的（地图索引x，地图索引y）; 
*/
...
/*
	3.收紧搜索边界，ShrinkToFit，边界参数存放在search_parameters中
*/
  search_parameters.ShrinkToFit(discrete_scans, limits_.cell_limits());

/*
	4.计算低分辨率地图中的候选粒子ComputeLowestResolutionCandidates，
	这种方法之所以能够快速进行匹配，也是利用了多分辨地图这个特点，分支定界就是在多分辨率地图中完成的。
	首先在低分辨率地图中，将旋转候选粒子以一定的平移补偿（x,y两个方向），
	放在整个搜索空间中（简单粗暴啊，三维空间中的搜索），然后按照每个候选粒子的平均分，按又高到低进行排序，
	得到排序后的lowest_resolution_candidates。 
*/
...
/*
	5.分支定界，BranchAndBound，下界min_score ，小于该得分的候选粒子直接就pass了。
	计算高一层分辨率地图所需要的粒子位置偏移量，然后在高一层分辨率地图中，再次计算每个粒子的平均得分并排序，
	最后递归;
*/
...
/*
可以看出递归采取深度优先的方式，先在某一个区域块（对应的就是低分辨率地图），
进行四分查找（二维，所以是四分，四分之后，地图分辨率提高一个层级），计算该区域块的最高得分的粒子，
然后递归下去，直到最高分辨率的地图，选择最高分辨率地图中，得分最高的粒子，其分数作为分支定界的下界，
然后再在其它区域进行这样的操作，直到扫描完整张地图。
*/
