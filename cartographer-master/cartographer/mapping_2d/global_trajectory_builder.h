...
/*
GlobalTrajectory类的声明;	继承mapping

3个变量:
1, options;-->声明了用local_trajectory_builder;
2, trajectory_id;
3, sparse_pose_graph; --->声明了检测类型；

成员函数:
1, mapping::GlobalTrajectoryBuilderInterface::PoseEstimate& pose_estimate();
2, AddRangefinderData;
3, AddImuData;
4, AddOdometerData;
*/
...
class GlobalTrajectoryBuilder
 ...
 public:
  GlobalTrajectoryBuilder(const proto::LocalTrajectoryBuilderOptions& options,
                          int trajectory_id,
                          SparsePoseGraph* sparse_pose_graph);      //构造函数
