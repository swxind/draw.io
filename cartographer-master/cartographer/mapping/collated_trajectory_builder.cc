...
/*
构造函数；
通过HandleCollatedSensorData，处理三大类消息；
*/
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    sensor::Collator* const sensor_collator, const int trajectory_id,
    const std::unordered_set<string>& expected_sensor_ids,
    std::unique_ptr<GlobalTrajectoryBuilderInterface> wrapped_trajectory_builder)  //消息处理的实体通过wrapped_tarjectory_builder指针调用，该指针定义为GlobalTrajectoryBuilderInterface
   ...
   : sensor_collator_(sensor_collator),
      ...
      last_logging_time_(std::chrono::steady_clock::now()) {
  												sensor_collator_->AddTrajectory(
        											...
        											HandleCollatedSensorData(sensor_id, std::move(data));     	//调用HandleCollatedSensorData；
    											        });
											  }
...
// 轮回处理3大消息类型；KImu, KRangefinder, KOdometer;
